...
def parse_args():
    p = argparse.ArgumentParser(description="Export Poe.com conversations with metadata and JSONL, and build a SQLite catalog.")
    p.add_argument("--build-db", action="store_true", help="Populate/update the catalog while exporting.")
    p.add_argument("--db-path", default=os.path.join(DEFAULT_OUTPUT_DIR, "catalog.sqlite"), help="Path to SQLite catalog (default ./output/catalog.sqlite).")
    p.add_argument("--since", help="Only (re)export pages updated since this ISO8601 timestamp.")
    p.add_argument("--index-only", action="store_true", help="Build/update DB from existing output folder without calling scraper.")
    p.add_argument("--output-dir", default=DEFAULT_OUTPUT_DIR, help="Output directory (default ./output).")
    return p.parse_args()
...
def scan_existing_output(output_dir: str) -> List[Dict[str, Any]]:
    ...
    for path in glob.glob(pattern):
        ...
        updated_times = [m.get("updated_at") for m in messages if m.get("updated_at")]
        ...
        results.append({
            ...
            "slug": conv_slug,
            "title": messages[0].get("section_title") or conv_slug.replace("-", " ").title(),
    ...
    with connect(args.db_path) as conn:
        ...
        for conv in scanned:
            ...
            upsert_conversation(conn, {
                ...
                "export_md_path": md_path if os.path.exists(md_path) else None,
                "export_assets_path": conv["assets_dir"] if os.path.isdir(conv["assets_dir"]) else None,
            ...
            for m in conv["messages"]:
                upsert_message(conn, {
                    ...
                    "export_md_path": os.path.join(conv_dir, f"{m.get('section_slug')}.md"),
...
def main():
    ...
    with connect(args.db_path if args.build_db else None) as maybe_conn:
        for conv in conversations:
            ...
            if res.get("skipped"):
                print(f"Skipped {conv.get('title') or conv['graph_id']}: {res.get('reason')}")
            else:
                print(f"Exported {conv.get('title') or conv['graph_id']} -> {res['export_paths']['md']}")
