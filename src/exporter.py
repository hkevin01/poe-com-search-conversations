...
def ensure_paths(base_output: str, conv_slug: str) -> tuple[str, str]:
...
def export_conversation_package(
    ...
    for idx, m in enumerate(messages, start=1):
        ...
        first_line = (
            (m.get("title") or m.get("content") or "")
            .strip()
            .split("\n")[0]
        )
        msg_title_fallback = first_line[:80] or f"message-{idx}"
        m["slug"] = m.get("slug") or slugify(
            f"{m['ordinal']:03d}-{msg_title_fallback}"
        )
        ...
        m["parent_graph_id"] = (
            messages[idx - 2]["graph_id"]
            if idx > 1
            else conversation.get("parent_graph_id")
        )
    ...
    if build_db and not index_only and conn:
        cur = conn.execute(
            "SELECT content_hash, updated_at FROM conversations "
            "WHERE graph_id = ?",
            (conv_graph_id,),
        )
        row = cur.fetchone()
