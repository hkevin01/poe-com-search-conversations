...
class ConversationDatabase:
    ...
    def search_conversations(self, query: str, filters: Dict[str, Any] = None) -> List[Conversation]:
        """Search conversations using robust FTS with fallback."""
        # Try using the new robust search backend first
        if search_messages:
            try:
                # Convert filters to the format expected by search backend
                bot_filter = filters.get('bot_name', '') if filters else ''
                date_from = filters.get('start_date', '') if filters else ''
                date_to = filters.get('end_date', '') if filters else ''
                limit = 50

                # Check if we're using the new catalog.sqlite format
                project_root = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
                catalog_path = os.path.join(project_root, "output", "catalog.sqlite")

                if os.path.exists(catalog_path):
                    # Use new catalog database
                    ...
                    if results:
                        ...
                        for row in results:
                            conv = Conversation(
                                id=row.get('id', 0),
                                poe_id=row.get('conversation_id', ''),
                                title=row.get('title', ''),
                                url=row.get('url', ''),
        ...
        # Legacy fallback - use simple LIKE search instead of broken FTS
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row

            if query.strip():
                # Use LIKE search instead of broken FTS
                sql = """
                    SELECT * FROM conversations
                    WHERE title LIKE ? OR content LIKE ? OR bot_name LIKE ?
                """
                search_term = f"%{query}%"
